...
actor_pg as (
select a.actor_id , a.first_name , a.last_name , f.title , f.film_id from actor_s as a 
inner join film_actor as fa on a.actor_id = fa.actor_id
inner join film as f on f.film_id = fa.film_id where rating = 'pg'), -- task 2 - filter film with rating as 'pg'
film_revenue as (
select ap.film_id , ap.title , p.amount from actor_pg as ap 
inner join inventory as i on ap.film_id = i.film_id 
inner join rental as r on r.inventory_id = i.inventory_id inner join payment as p on p.rental_id = r.rental_id
...
else 'common' end as film_availabiliy 
from film;
